...
# Set up paths
BARSEQ_INPUT_DIR <- '/data/barseq_780346_2025-06-13_12-00-00_processed_MAT2RDS_2026-05-28_17-50-51/BARseq/'  # read-only
...
################## Perform BARseq clustering (PCA-based for accuracy) - handles 800K dataset ##################
analyze_barseq_pca_only <- function(barseq, output_name, n_pca = 20, k_cluster = 30) {
  output_dir <- file.path("analysis", make.names(output_name))
  dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
  
  # Run PCA with reduced dimensions
  barseq <- scater::runPCA(barseq, ncomponents = n_pca, exprs_values = "logcounts")
  
  # Cluster on PCA (no UMAP)
  clusters <- cluster_data(barseq, k_cluster, "jaccard")  # Uses original cluster_data on PCA
  write_csv(clusters, file.path(output_dir, "cluster.csv"))
...
clear_objects_except_functions <- function() {
  ...
  if (dir.exists("/results")) {
    cat(sprintf("[%s] clear_objects (combined): RECOMPUTE_CLUSTERING before=%s after=%s\n",
...
##################  Function to create the plot with cells on slices ################## 
create_plot <- function(data, title_suffix = "") {
  plot <- ggplot(data, aes(x = CCF_ML, y = -CCF_DV, color = louvain_cluster)) +  # Use -CCF_DV to invert the y-axis
    geom_point(alpha = 0.8, size = 0.2) +  # Make dots smaller by reducing size
    ...
    labs(
      ...
      title = paste("LC Cluster Cells Slices", title_suffix)
    ) +
    theme_minimal() +
    theme(
