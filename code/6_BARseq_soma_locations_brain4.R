...
for (s in slices_LCNE) {
  ...
  p <- ggplot() +
    ...
    theme(axis.line = element_blank(),
          ...
          axis.title = element_blank()) +
    scale_color_manual(
      values = c("#dcbeff","#aaffc3", "#fabed4",  "#e6194B", "#911eb4"),
      name = "cluster",
      breaks = c("1", "2", "3", "4"),
...
p_coronal_louvain <- ggplot() +
  ...
  scale_color_manual(
    ...
    name = "cluster",
    breaks = c("1", "2", "3", "4", "5"),
    labels = c("LC_1", "LC_2", "LC_3", "LC_4", "LC_5")
  ) +
  labs(x = "CCF_ML (reflected)", y = "CCF_DV")
...
# Create a named vector of colors and color vector based on the characteristic
color_map_louvain_cluster <- c("1"="#dcbeff", "2"="#aaffc3", "3"="#fabed4","4"="#e6194B","5"="#911eb4", "other"="lightgrey")
