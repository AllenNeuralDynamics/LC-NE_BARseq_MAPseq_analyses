...
load_data <-function() {
  ...
  original_rows <- nrow(projection_matrix)
  
  # Remove duplicates based on CellID, keeping first occurrence
  projection_matrix <- projection_matrix[!duplicated(projection_matrix$CellID), ]
  duplicates_removed <- original_rows - nrow(projection_matrix)
  ...
  if(duplicates_removed > 0) {
    cat("✓ Removed", duplicates_removed, "duplicate CellIDs\n")
  } else {
  ...
  proj_index <- readr::read_tsv("/data/barseq_780345_2025-02-24_12-00-00_processed_MAT2RDS_2026-05-28_17-49-02/MAPseq/sampleinfo_780345.tsv")
