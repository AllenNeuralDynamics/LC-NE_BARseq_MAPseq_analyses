...
cell_factors <- cell_factors %>%
  mutate(
    proj_pattern = ifelse(factor %in% c(5), "Posterior", "Anterior"),
    proj_target  = case_when(
      factor == 1 ~ "ctx-anterior",
      factor == 5 ~ "medulla-SP",
      factor == 3 ~ "olf",
      factor == 2 ~ "midbrain-hindbrain",
      factor == 4 ~ "ctx-posterior_hippocampus",
      TRUE        ~ NA_character_
    )
  )
...
mean_expr_by_factor <- matrix(0, nrow = nrow(expr_matrix_sorted), ncol = 5)
rownames(mean_expr_by_factor) <- rownames(expr_matrix_sorted)
colnames(mean_expr_by_factor) <- paste("Factor", 1:5)

# Also calculate sample sizes for each factor
factor_sizes <- numeric(5)
for(f in 1:5) {
...
print(setNames(factor_sizes, paste("Factor", 1:5)))
...
proj_targets <- c("ctx-anterior",
                  "midbrain-hindbrain",
                  "olfactory",
                  "ctx-posterior_hippocampus",
                  "medulla-SP")
colnames(mean_expr_zscore) <- paste0("Factor ", 1:5, "\n(", proj_targets, ")")
...
factor_specific_df <- data.frame(
  Factor = rep(1:5, sapply(factor_specific_genes, length)),
...
top_genes_df <- data.frame(
  Factor = rep(1:5, each = 10),
  Projection_Target = rep(proj_targets, each = 10),
  Gene = unlist(lapply(1:5, function(f) {
