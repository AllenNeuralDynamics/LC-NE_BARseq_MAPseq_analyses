...
################## Make sure correct CPM normalization was performed and used for log conversion ##################
check_file <- function(path, assay_name = "cpm") {
  sce <- readRDS(path)
  ...
  if (assay_name %in% assayNames(sce)) {
    cs <- Matrix::colSums(assay(sce, assay_name))
    cat("colSums(", assay_name, ") summary:\n", sep="")
    print(summary(cs))
  } else {
    cat("No assay named", assay_name, "\n")
  }
  
  if ("logcounts" %in% assayNames(sce) && assay_name %in% assayNames(sce)) {
    # Compare a small block for exact consistency
    i <- 1:min(50, nrow(sce))
    j <- 1:min(200, ncol(sce))
    lc_expected <- log1p(assay(sce, assay_name))/log(2)
    maxdiff <- max(abs(assay(sce, "logcounts")[i, j] - lc_expected[i, j]))
    cat("max abs diff (stored logcounts vs log2(1+", assay_name, ")):", maxdiff, "\n", sep="")
    cat("logcounts range (small block):\n")
    print(range(assay(sce, "logcounts")[i, j]))
  } else {
    cat("Missing logcounts and/or", assay_name, "assay; cannot validate.\n")
  }
