...
get_base_region <- function(col_name) {
  # remove -contra.NUM or -ipsi.NUM OR trailing .NUM
  name <- sub("-(contra|ipsi)\\.\\d+$", "", col_name)  # remove contra/ipsi forms
  name <- sub("\\.\\d+$", "", name)                    # then remove trailing index (SP form)
  name
}
