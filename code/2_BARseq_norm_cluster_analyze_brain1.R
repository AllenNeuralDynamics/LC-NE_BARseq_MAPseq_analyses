...
# Load functions
source("~/capsule/code/1_BARseq_analyses_functions_brain1.R")
...
logcounts(all_cells) <- log1p(assay(all_cells, "cpm"))/log(2) # depending on state quierries wrong assay - logcounts(all_cells) <- log1p(cpm(all_cells))/log(2)
assayNames(all_cells)
saveRDS(all_cells, file.path(BARSEQ_OUTPUT_DIR, "combined_neurons_clust_CCFv2_uid_cpm_log.rds"))
out_file <- file.path(BARSEQ_OUTPUT_DIR, "combined_neurons_clust_CCFv2_uid_cpm_log.rds")
check_file(out_file, assay_name = "cpm")
rm(all_cells)
...
gene_expression <- logcounts(barseq)[genes_of_interest, cluster_11_cells]
...
expr_matrix <- logcounts(LC_barseq)
# Extract cluster labels
cluster_labels <- colData(LC_barseq)$louvain_cluster
...
rownames(expr_matrix_sorted) <- rownames(LC_barseq)
...
logcounts_matrix <- logcounts(LC_barseq)  # Extract log-normalized counts
# Create a data frame with cluster labels and logcounts
expression_data <- as.data.frame(t(logcounts_matrix))  # Transpose to make cells rows
expression_data$cluster <- cluster_labels
# Calculate mean expression for each gene by cluster
